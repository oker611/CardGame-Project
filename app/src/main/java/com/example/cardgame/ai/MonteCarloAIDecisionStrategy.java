...
public class MonteCarloAIDecisionStrategy implements AIDecisionStrategy {
    ...
    private double applyAdaptiveFactors(double score, Play candidate, Play lastPlay,
        ...
        Log.d(TAG, "[MonteCarloAI] applyAdaptiveFactors - score=" + score +
                " agg=" + aggressivenessFactor + " def=" + defenseFactor);
        CandidateScorer scorer = new CandidateScorer(aggressivenessFactor, defenseFactor);
        return scorer.applyFactors(score, candidate, lastPlay, gameState, aiPlayer);
    }

    // Keep for backward compatibility — replaced by CandidateScorer
